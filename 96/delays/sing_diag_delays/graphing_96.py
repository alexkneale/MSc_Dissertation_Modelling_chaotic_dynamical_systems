...
MODEL_DIR = (f"models/new_NC{N_C}_{delay}_{n_steps_train_str}_{LEARNING_RATE_str}_{BATCH_SIZE}_{PATIENCE}_{test_steps_str}_{K}_{J}_{train_particles_str}_{test_particles_str}_{h}_{b}_{c}_{F}/")
...
plt.plot(midx_nn_valid, histogram_nn_valid, '--', label='SD Model')
plt.xlim(known_min-1, known_max+1)  # Set x-axis limits to the known range
plt.title('Distribution of $x_i$, SD Model')
plt.xlabel('$x_i$')
plt.ylabel('$p(x_i)$')
plt.legend()
plt.grid(True)
...
plt.plot(lags, temp_autocorr_nn, marker='v',label='SD Model')
...
plt.plot(lags, space_autocorr_nn, marker='v',label='SD Model')    
