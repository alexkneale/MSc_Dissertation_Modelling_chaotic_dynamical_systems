...
plt.figure(6)
# plotting trajectory
plt.plot(traj_96[:3*10**4,0],'--',label='trajectory')
plt.title('$x_i$ trajectory')
plt.ylabel('$x_i$')
plt.xlabel('timestep')
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'x_traj.pdf'))
plt.close()
...
def lorenz_96_data_display(traj_96,traj_nn, max_lag_temp,max_lag_space,dt,output_dir,n_particles,K,J,n_steps_train,train_particles,test_steps,test_particles):
    ...
    plt.figure(2)
    # truth
    histogram_96,bins_96 = np.histogram(traj_96[:,:K].flatten(),bins=400, density=True)
    midx_96 = (bins_96[0:-1]+bins_96[1:])/2    
    # nn
    histogram_nn,bins_nn = np.histogram(traj_nn[:,:K].flatten(),bins=400, density=True)
    midx_nn = (bins_nn[0:-1]+bins_nn[1:])/2  
    # plotting
    plt.plot(midx_96,histogram_96,'--',label='Truth')
