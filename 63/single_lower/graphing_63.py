...
MODEL_DIR = (f"models/MD_NC{N_C}_{n_steps_train_str}_{LEARNING_RATE_str}_{BATCH_SIZE}_{PATIENCE}_{test_steps_str}_{str_n_particles_train}_{str_n_particles_test}/")
...
plt.title('Distribution of $x$, SL Model')
plt.xlabel('$x$')
plt.ylabel('$p(x)$')
plt.legend()
plt.grid(True)
...
plt.plot(midx_63,histogram_63,'--',label='Truth')
plt.plot(midx_nn,histogram_nn,'--',label='SL Model')
plt.title('Distribution of $y$, SL Model')
plt.xlabel('$y$')
plt.ylabel('$p(y)$')
plt.legend()
plt.grid(True)
...
plt.plot(lags, autocorr_x_nn, marker='v',label='SL Model')
...
plt.plot(lags, autocorr_y_nn, marker='v',label='SL Model')    
...
plt.plot(lags, autocorr_z_nn, marker='v',label='SL Model')
