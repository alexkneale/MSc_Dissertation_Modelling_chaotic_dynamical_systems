...
# temporal autocorrelation
autocorr_x_nn = pd.read_csv(f'{MODEL_DIR}/temp_autocorr_nn_x.csv')
...
plt.plot(midx_nn,histogram_nn,'--',label='Deterministic Model')
plt.title('Distribution of $x$, Deterministic Model')
plt.xlabel('$x$')
plt.ylabel('$p(x)$')
plt.legend()
plt.grid(True)
...
plt.plot(midx_63,histogram_63,'--',label='Truth')
plt.plot(midx_nn,histogram_nn,'--',label='Deterministic Model')
plt.title('Distribution of $y$, Deterministic Model')
plt.xlabel('$y$')
plt.ylabel('$p(y)$')
plt.legend()
plt.grid(True)
...
plt.plot(lags, autocorr_x_nn, marker='v',label='Deterministic Model')
...
plt.plot(lags, autocorr_y_nn, marker='v',label='Deterministic Model')    
...
plt.plot(lags, autocorr_z_nn, marker='v',label='Deterministic Model')
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'determinant_zoom'))
plt.close()
'''
